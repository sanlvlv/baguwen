...
栈和堆都是进程内存空间中的区域，但用途不同。

栈主要用于函数调用。每次函数调用都会创建一个栈帧，里面保存返回地址、局部变量、参数等信息。函数调用结束后，栈帧会自动销毁，所以栈由系统自动管理，分配和释放速度很快，但空间通常比较小，递归太深可能导致栈溢出。

堆主要用于动态内存分配。程序运行过程中创建的大对象、生命周期不确定的对象一般放在堆上。堆的空间通常比栈大，但分配和释放相对慢一些，因为需要维护空闲内存块，也可能产生内存碎片。在 C/C++ 中堆内存通常需要程序员手动申请和释放，而在 Python 中对象通常分配在堆上，由解释器和垃圾回收机制管理。
...
<br><br>
## 04.深拷贝和浅拷贝的区别是什么
深拷贝和浅拷贝都是复制对象，但复制的层级不同。

浅拷贝只复制对象的最外层容器，内部元素仍然引用原来的对象。比如一个二维列表，浅拷贝后，外层列表是新的，但是里面的子列表还是和原对象共享的。所以如果修改外层结构，比如 append 一个新元素，原对象不受影响；但如果修改内部子列表，原对象也会受到影响。

深拷贝会递归复制对象内部的子对象，外层和内层都会尽量创建新的对象。这样新对象和原对象相互独立，修改嵌套结构时不会互相影响。

在 Python 里，可以用 `copy.copy()` 做浅拷贝，用 `copy.deepcopy()` 做深拷贝。另外，如果对象是不可变对象，比如整数、字符串，拷贝时可能会直接复用原对象，因为不可变对象本身不能被修改，没必要复制一份。
...
面向对象主要有三大特性：封装、继承和多态。

封装是指把对象的属性和方法按照职责放到一个类里面，同时隐藏一些不希望外部直接访问的细节。这样可以让代码结构更清晰，也可以避免外部随意修改对象内部状态。

继承是指子类可以复用父类已有的属性和方法，避免重复编写相同代码。子类在继承父类的基础上，也可以扩展自己的方法，或者重写父类的方法。

多态是指同一个方法调用，作用在不同对象上会产生不同的结果。比如 `Dog` 和 `Cat` 都有 `speak()` 方法，同样调用 `speak()`，狗会输出“汪汪汪”，猫会输出“喵喵喵”。它的好处是调用方只需要面向统一接口编程，不需要关心具体对象类型。
...
## 06.什么是闭包
`高级` `Python`<br><br>
&#8195;&#8195;闭包可以理解为“函数加上它引用的外部环境变量”。在 Python 里，如果一个函数内部又定义了一个函数，并且内部函数引用了外部函数中的局部变量，那么当外部函数返回内部函数后，这个局部变量不会随着外部函数结束而销毁，而是会被内部函数保存下来，这就形成了闭包。

闭包的作用主要是保存状态。比如可以用闭包实现计数器，也可以用闭包实现装饰器。它的好处是可以避免使用全局变量，同时又能让函数记住某些数据。
...
这里 `inner` 使用了外部函数 `outer` 的变量 `x`。当执行 `f = outer(10)` 后，`outer` 已经结束了，但 `f(5)` 仍然可以使用 `x = 10`，所以 `inner` 和 `x` 就组成了闭包。
...
匿名函数和普通函数作为实参时，传递的是函数对象的引用，主要体现的是一段可执行功能。

匿名函数适合简单临时逻辑，普通函数适合更清晰、可复用的逻辑。

闭包作为实参时，传递的不只是内部函数，还包括它引用的外部环境变量，所以可以理解为“功能 + 数据”。

对象作为实参时，可以携带属性和方法，因此可以表示更复杂的状态和行为。

如果对象实现了 `__call__` 方法，还可以像函数一样被调用。
...
- IO密集型：用线程或协程
  ...
  如果是计算密集型任务，比如大量数学计算、图像处理、视频编码、加密解密等，主要瓶颈是 CPU。在 Python 里由于 GIL 的存在，多线程很难真正并行执行 Python 代码，所以更适合使用多进程。多进程有独立的解释器和内存空间，可以更好地利用多核 CPU。

  如果是 IO 密集型任务，比如网络请求、文件读写、数据库查询、Redis 访问等，主要瓶颈不是 CPU，而是等待外部资源返回。这时可以使用多线程或者协程。线程可以在一个线程等待 IO 时切换到其他线程执行；协程则通过 `async/await` 在等待 IO 时主动让出执行权，切换成本更低，适合高并发 IO 场景。
...
## 11.协程为何比线程还快
`高级` `Python`<br><br>
&#8195;&#8195;高并发+高扩展性+低成本：一个CPU支持上万的协程都不是问题。所以很适合用于高并发处理，协程能保留上一次调用时的状态，不管是进程还是线程，每次阻塞、切换都需要陷入系统调用，使用线程时需要非常小心地处理同步问题，而协程完全不存在这个问题。
...
## 14.简述迭代器、生成器的区别
`高级` `Python`<br><br>
&#8195;&#8195;迭代器是 Python 中用来遍历数据的一种对象，它会保存当前遍历的位置，可以通过 `next()` 方法一个一个取出元素。像列表、元组、字符串这些是可迭代对象，可以通过 `iter()` 得到对应的迭代器。

生成器是一种特殊的迭代器，通常通过 `yield` 实现。它不会一次性把所有数据都生成出来，而是在每次调用 `next()` 时，从上一次暂停的位置继续执行，遇到下一个 `yield` 时返回一个值并再次暂停。所以生成器具有惰性计算的特点，适合处理大量数据或者数据流场景，可以节省内存。
...
## 15.什么是装饰器，为什么要使用它
`高级` `Python`<br><br>
- 装饰器本质上是一个函数，它接收一个函数作为参数，在内部定义一个包装函数，在包装函数中添加额外功能，然后再调用原函数，最后返回这个包装函数。

  使用装饰器的目的，是在不修改原函数代码、不改变原函数调用方式的情况下，动态增强函数功能。比如可以用装饰器实现日志记录、权限校验、运行时间统计、缓存、事务控制等公共功能。
  ...
  所以调用 `add(1, 2)` 时，实际执行的是包装后的 `wrapper`，它会先执行增强逻辑，再执行原来的 `add` 函数。

  一句话总结：**装饰器就是在不改原函数代码的情况下，给函数增加额外功能的一种机制。**
...
第一种是当前线程执行 Python 字节码执行了一段时间后，解释器会根据线程切换间隔尝试切换线程，让其他线程也有机会获取 GIL 执行。

第二种是线程遇到 IO 阻塞，比如网络请求、文件读写、数据库访问等。因为等待 IO 的时候线程不需要继续执行 Python 字节码，所以会释放 GIL，让其他线程执行。这也是 Python 多线程适合 IO 密集型任务的原因。
...
Python 高并发方案主要看任务类型。如果是 IO 密集型任务，比如网络请求、数据库访问、文件读写、WebSocket 长连接等，可以通过多线程、协程或异步框架来提升并发能力。多线程适合中小规模 IO 并发，使用简单；`asyncio` 是 Python 标准库中的异步 IO 方案，通过事件循环和 `async/await` 在 IO 等待时切换任务，是现在比较主流的方案。

除了标准库 `asyncio`，还有一些框架方案。`gevent` 基于 greenlet，可以通过 monkey patch 把同步 IO 代码改造成协程并发，适合改造老项目；`Twisted` 是老牌事件驱动网络框架，稳定、协议支持丰富，但学习成本较高；`Tornado` 是异步 Web 框架和网络库，适合 WebSocket、长连接、长轮询等场景。

在现代 Web API 项目里，更常见的是 `FastAPI + Uvicorn`。FastAPI 负责业务接口，Uvicorn 作为 ASGI Server 负责处理异步请求。IO 并发靠 `async/await`，多核 CPU 利用可以通过多个 worker 进程实现。
...
`==` 比较的是两个对象的值是否相等，它底层通常会调用对象的 `__eq__` 方法，所以不同对象只要内容相同，`==` 也可以返回 True。

`is` 比较的是两个变量是否引用同一个对象，也就是对象身份是否一致，可以理解为比较它们的 `id` 是否相同。`is` 不关心对象内容是否相等，也不能被重载。

[返回目录](#目录) 
<br><br>
## 26.`__new__()` 和 `__init__()`的区别
`高级` `Python`<br><br> 
&#8195;&#8195;`__new__()` 是对象创建方法，它在 `__init__()` 之前执行，第一个参数是 `cls`，表示当前类。它的作用是创建并返回一个对象。如果 `__new__()` 返回的是当前类的实例，Python 才会继续调用 `__init__()`。

`__init__()` 是对象初始化方法，它在对象创建之后执行，第一个参数是 `self`，表示已经创建好的实例对象。它主要用于给对象设置属性，不能返回非 `None` 的值。

所以简单来说，`__new__()` 负责创建对象，`__init__()` 负责初始化对象。平时写类通常只需要重写 `__init__()`；只有在单例模式、继承不可变类型、控制对象创建过程等场景下，才会重写 `__new__()`。
...
# Linux模块
## 01.列举10个常见的Linux命令
`初级` `Linux`<br><br>目录相关的命令有 `pwd`、`cd`、`ls`、`mkdir`。`pwd` 用来查看当前路径，`cd` 用来切换目录，`ls` 用来查看目录内容，`mkdir` 用来创建目录。

文件相关的命令有 `touch`、`cat`、`more`、`cp`、`mv`、`rm`。`touch` 可以创建空文件，`cat` 和 `more` 用来查看文件内容，`cp` 用来复制文件，`mv` 可以移动或重命名文件，`rm` 用来删除文件。

查找和过滤相关的命令有 `find` 和 `grep`。`find` 用来按照文件名、类型等条件查找文件，`grep` 用来在文本中搜索关键字。

系统相关的命令有 `ps`、`date`、`clear`、`help`、`sudo`。`ps` 用来查看进程，`date` 查看时间，`clear` 清屏，`help` 查看帮助，`sudo` 用管理员权限执行命令。

远程相关的命令有 `ssh` 和 `scp`。`ssh` 用来远程登录服务器，`scp` 用来在本地和远程服务器之间复制文件。`vim` 是终端里的文本编辑器，用来编辑配置文件或代码文件。
